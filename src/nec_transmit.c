...
int nec_tx_init(PIO pio, uint pin_num) {
    ...
    if (pio_can_add_program(pio, &nec_carrier_program)) {
        carrier_burst_offset = pio_add_program(pio, &nec_carrier_program);
    } else {
        return -1;
    }
    ...
    if (pio_can_add_program(pio, &nec_transmit_program)) {
        carrier_control_offset = pio_add_program(pio, &nec_transmit_program);
    } else {
        return -1;
    }
    ...
    // configure and enable the state machine
    nec_transmit_program_init(pio,
                                     carrier_control_sm,
                                     carrier_control_offset,
                                     2 * (1 / 562e-6f),        // 2 ticks per 562.5us carrier burst
